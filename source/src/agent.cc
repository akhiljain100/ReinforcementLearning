...
int getMin(std::map<int, double> mymap) 
{
...
{
double reward = 0; 
  if(g_value[agentstate][agentstate]== exp(-0))
          {
             reward= 100000;
          }
        else if(g_value[agentstate][agentstate]== exp(-1))
             reward=-0.1; 
        else
          {
             reward = -2;
...
{   vector <double> validActions;
  double maxe = 0;
    ...
    if(nextstate !=120)
    {
  auto it = max_element(begin(validActions), end(validActions));
  maxe=*it;
  }
  else{
    max=0;
...
void Agent::randomQlearning()
{
 for(int n = 0 ; n < num_iter; n++)
  {
  double tot_step=0,step = 0,reward,alpha,gamma=0.7,maxElement;
  ...
  while(tot_step < tot_steps)
  {
      ...
      if(nextstate == 3)
      {
        cout << "\n Reward for 3 state" << reward; 
      }
      
      maxElement = findValidMaxElement(nextstate);
      if(agentState == 3)
      {
        cout << "\nreward" << reward << "gamma" << gamma << "maxElement" << maxElement << "\n"; 
      cout << "\n Agent state 3 q value " <<  q_value[agentState][action] << "  action  " << action << "update value" << reward+ gamma * maxElement << "\n";}
	  //cout <<"\n Max element"<< maxElement << "\n";
	  //cout << "\ncurrentstate  " << agentState << "  q_value  " << q_value[agentState][action]<< "  nextstate  " << nextstate << " totstep "<< tot_step; 			
      q_value[agentState][action] = q_value[agentState][action] + alpha * (reward+ gamma * maxElement- q_value[agentState][action]);
     ...
     /* for(int i=0;i<grid.ROW;i++)
    {
      ...
      for(int j=0;j<grid.COLUMN;j++)
      {
        auto it = max_element(begin(q_value[i*grid.COLUMN+j]), end(q_value[i*grid.COLUMN+j]));
            
        q_val.push_back(*it);
      }
      q_value_state.push_back(q_val);
      q_val.clear();
    }*/
 ...
 /*  string file = "../Result/q_value_eachepisode.txt";
  
  ofstream outfileQvalue(file);
  for(int i=0;i<q_value_state.size();i++){

  for(int j=0;j<q_value_state[i].size();j++){
